package com.vidiun.core {
	...
	public class VClassFactory {
				...
				if (prop.hasComplexContent()) {
					...
					if (propChildren[0].name() == "item") {
						...
						for each (var item:XML in propChildren) {
							fallbackClass = '';
							if (instance.hasOwnProperty('getElementType')) fallbackClass = instance['getElementType'](propName);
							instance[propName].push(setObject(item.objectType, item, fallbackClass));
						}
					}
					else //if complex object and not an array
					{
						...
						try {
							fallbackClass = '';
							if (instance.hasOwnProperty('getElementType')) fallbackClass = instance['getElementType'](propName);  
							newComplexObject = setObject(prop.objectType, prop, fallbackClass);
		 ...
		 * @param fallbackType	type to create if failed to create objectType
		 * @return the new object
		 */
		private function setObject(objectType:String, xmlInfo:XML, fallbackType:String = ''):Object {
			var cls:Class;
			try {
